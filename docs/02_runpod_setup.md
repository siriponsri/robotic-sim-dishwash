...
NB06–NB08 (Training PPO/SAC/Residual) ต้องใช้ **GPU** เพื่อ:
- เปิด simulation หลาย env พร้อมกัน (vectorized env)
- คำนวณ neural network ได้เร็ว
- เทรน 500K steps ภายในเวลาที่เหมาะสม (1–3 ชม. แทน 1–2 วัน บน CPU)
...
|-----------|---------|-------|---------|
| **GPU** | RTX 3090 (24 GB) | **RTX 4090** (24 GB) | เร็วกว่า ~2x |
| **VRAM** | 12 GB | 24 GB+ | Replay buffer + model weights |
| **CPU Cores** | 8 | 16+ | สำหรับ vectorized env |
| **RAM** | 32 GB | 64 GB | Replay buffer (SAC) ใช้เยอะ |
| **Storage** | 20 GB | 50 GB+ | Model checkpoints + artifacts |
...
2. เลือก **GPU**: `RTX 4090` (หรือ 3090/A100 ตามงบ)
...
```
1. NB01 — Smoke test (ตรวจว่า env ใช้ได้ + GPU ทำงาน)
2. NB05 — Baselines (ได้ BaseController สำหรับ NB08)
3. NB06 — Train PPO      ← ใช้เวลา 1-3 ชม.
4. NB07 — Train SAC       ← ใช้เวลา 1-3 ชม.
5. NB08 — Residual SAC    ← ใช้เวลา 3-6 ชม. (3 runs)
6. NB09 — Evaluation (load models จาก NB06/07/08)
```

> **Tip**: NB06 และ NB07 ไม่ต้องพึ่งกัน — สามารถรันพร้อมกันได้ (ถ้ามี VRAM พอ)
...
```bash
git add artifacts/NB06/
git add artifacts/NB07/
git add artifacts/NB08/
git commit -m "feat: add trained models PPO/SAC/Residual"
...
```bash
# บนเครื่อง local
scp -P 12345 root@123.456.789.0:/workspace/robotic-sim-dishwash/artifacts/NB06/ppo_model.zip ./
```
...
**สาเหตุ**: VRAM ไม่พอ  
**แก้**:
- ลด `N_ENVS` ใน config (เช่น จาก 4 เป็น 2)
- ลด `batch_size`
- ลด `buffer_size` (SAC)
